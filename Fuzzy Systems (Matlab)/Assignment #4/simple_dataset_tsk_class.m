...
% Dataset: The dataset contains cases from a study that was conducted...
...between 1958 and 1970 at the University of Chicago's Billings...
...Hospital on the survival of patients who had undergone surgery for...
...breast cancer.
...
for i=1:length(labels)
  ...
  bounds_per_label{end+1} = ...
      [min(data_tr_label)-10^(-12); max(data_tr_label)];
...
for kc = 1:length(clust_radius)
    ...
    % calculate metrics
    [errmat, OA, UA, PA, Khat] = evaluate_class( yout,...
      data_test(:,n_f) );
    ...
    text = ...
        sprintf('Learning Curve for TSK Class dependent, Radius: %f ',...
      clust_radius(kc)); 
    title(text);
    ...
    for l=1:n_f-1
      ...
      txt = ...
          sprintf('Membership functions Class Dependent, Radius: %.2f ',...
        clust_radius(kc));
      title(txt);
    end
...
for kc = 1:length(clust_radius)
  ...
  % calculate metrics
  [errmat, OA, UA, PA, Khat] = evaluate_class( ypred, data_test(:,n_f) );
  ...
  plot(1:num_epoch, trErr);
  text = ...
      sprintf('Learning Curve for TSK Class Independent , Radius: %f ',...
    clust_radius(kc)); 
  title(text);
  ...
  for l=1:n_f-1
    ...
    plotmf(model_c,'input',l);
    txt = ...
        sprintf('Membership functions Class Independent, Radius: %.2f ',...
        clust_radius(kc));
    title(txt);
  end

  txt = sprintf('Metrics - Class Independent , Radius: %f ',...
    labels(i),clust_radius(kc));
  disp(txt);
  errmat
  OA
