...
void *connection_handler(void *socket_desc)
{
  ...
  struct timeval rectime; // this structure is used to specify when a the message was sent
  ...
  //long timestamp;
  //FILE *file;
  //file = fopen("time.txt","w");	// creating a file to insert the results
  ...
  */
  ...
  if (r_address[0] == 'C'){
	...
	sum[rnum-1]++;
  	response = 1;
        //gettimeofday(&rectime,NULL);		// timestamping when the message was sent
        //clock_gettime(CLOCK_REALTIME, &rectime);
        //sec = rectime.tv_sec;
        //ms = rectime.tv_usec / 1.0e6;
        //timestamp = sec + ms;
        //timestamp = rectime.tv_usec/1.0e6 + rectime.tv_sec;
	//timestamp = (double) rectime.tv_usec/1.0e6 + rectime.tv_sec;
	//timestamp = 5;
        //fprintf(file,"%f\n",timestamp);
        //fclose(file);
        //printf("The timestamp is: %ld");
        n = send(sock,"R",2,0);			// sending the ACK message which is the 'R'
